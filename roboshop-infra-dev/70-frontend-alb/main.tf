...
resource "aws_lb_listener" "frontend_alb" {
  load_balancer_arn = module.frontend_alb.arn #arn means amzon resource name-aws loadbalancer check the arn anme.
  port              = "443"
  protocol          = "HTTPS"
...
resource "aws_route53_record" "frontend_alb" {
  zone_id = var.zone_id
  name = "${var.environment}.${var.zone_name}"  #dev.kimidi.site 
  type = "A"
