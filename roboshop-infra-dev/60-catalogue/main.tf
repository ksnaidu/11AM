...
resource "aws_ec2_instnce_state" "catalogue" {
...
# aws AMI through instance terraform 

resource "aws_ami_from_instance" "catalogue" {
  name = "${var.project}-${var.environment}-catalogue"
  source_instance_id = aws_instance.catalogue.id
  depends_on = [ aws_ec2_instnce_state.catalogue ]  ## instnace stop then we can take AMI
  tags = merge (
    local.common_tags,
    {
      Name = "${var.project}-${var.environment}-catalogue"
...
resource "aws_launch_template" "catalogue" {
  ...
  image_id = aws_ami_from_instance.catalogue.idresource "aws_lb_target_group" "catalogue" {
  name     = "${var.project}-${var.environment}-catalogue" #roboshop-dev-catalogue
  port     = 8080
  protocol = "HTTP"
  vpc_id   = local.vpc_id
